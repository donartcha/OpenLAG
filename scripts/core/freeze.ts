...
export function renderTypstFreeze(profile: ExportProfile, artifacts: ParsedArtifact[], relations: ParsedRelation[]): string {
  const includeToc = profile.rendering?.includeTableOfContents !== false;
  const includeRelationTables = profile.rendering?.includeRelationTables !== false;
  const includeSourceMetadata = profile.rendering?.includeSourceMetadata === true;

  const lines = [
    '#set document(title: "' + profile.name + '")',
  ...
  if (includeToc) {
    lines.push('#outline(title: "Table of Contents", depth: 2)', '#pagebreak()', '');
  }
  ...
  for (const section of profile.sections) {
    ...
    for (const artifact of sectionArtifacts) {
      lines.push(`=== ${artifact.title || artifact.id}`, '');
      ...
      lines.push('', artifact.description ? artifact.description.replace(/\\/g, '\\\\').replace(/#/g, '\\#').replace(/\*/g, '\\*').replace(/`/g, '\\`').replace(/_/g, '\\_') : '_No description._', '');

      const outgoing = relations.filter((relation) => relation.from === artifact.id);
      if (outgoing.length > 0 && includeRelationTables) {
        lines.push('#table(');
...
export function createDocumentationFreeze(options: FreezeOptions): FreezeResult {
  ...
  if (format !== 'markdown' && format !== 'pdf' && format !== 'typst') {
    throw new Error(`Unsupported freeze format "${format}". Supported formats are: markdown, pdf, typst.`);
  }
  ...
  const markdown = format === 'markdown' 
    ? renderMarkdownFreeze(profile, sortArtifacts(artifacts), sortRelations(relations))
    : renderTypstFreeze(profile, sortArtifacts(artifacts), sortRelations(relations));
    
  let outputFile = resolveOutputFile(projectRoot, profile, options.output);
  if (format === 'pdf') {
    outputFile = outputFile.replace(/\.md$/, '.pdf');
  } else if (format === 'typst') {
    outputFile = outputFile.replace(/\.md$/, '.typ');
  }
  ...
  if (!options.dryRun) {
    ...
    if (format === 'pdf') {
      const typFile = outputFile.replace(/\.pdf$/, '.typ');
      fs.writeFileSync(typFile, markdown, 'utf-8');
      
      console.log('Compiling PDF using Typst...');
      execSync(`npx typst compile "${typFile}" "${outputFile}"`, { stdio: 'inherit' });
    } else {
