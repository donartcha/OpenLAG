...
export async function initProject(
  ...
  console.log(chalk.blue('Initializing OpenLAG for: ') + chalk.bold(name));
  ...
  // Copy base profile (core by default, starter for OpenLAG Lite)
  const baseProfile = starter ? 'starter' : 'core';
  const baseProfileDir = path.join(packageRoot, 'profiles', baseProfile);
  if (fs.existsSync(baseProfileDir)) {
    const profileLabel = baseProfile === 'starter' ? 'Starter' : 'Core';
    console.log(chalk.blue(`Applying ${profileLabel} Profile...`));
    applyProfilePack(baseProfile, ROOT_DIR);
    console.log(chalk.green(`${profileLabel} contracts initialized`));
  } else {
    console.warn(chalk.yellow(`${baseProfile} profile not found at ${baseProfileDir}`));
  }
  ...
  if (profile) {
    ...
    if (!fs.existsSync(profileDir)) {
      console.warn(chalk.yellow(`Profile pack '${profile}' not found in ${profileDir}. Skipping profile application.`));
    } else {
